...
/// SPI peripheral operating in full duplex master mode and which transfers data without checking
/// status registers. This sacrifices error detection but improves bandwidth.
pub struct UncheckedSpi<SPI, PINS> {
    spi: SPI,
    pins: PINS,
}
...
macro_rules! hal {
    ($($SPIX:ident: ($powerDomain:ident, $spiX:ident),)+) => {
        $(
            ...
            impl<SCK, MISO, MOSI> UncheckedSpi<$SPIX, (SCK, MISO, MOSI)> {
                /// Configures the SPI peripheral to operate in full duplex master mode
                pub fn $spiX<F>(
                    spi: $SPIX,
                    pins: (SCK, MISO, MOSI),
            ...
            impl<PINS> FullDuplex<u8> for UncheckedSpi<$SPIX, PINS> {
                type Error = Error;
            ...
            impl<PINS> FullDuplex<u16> for UncheckedSpi<$SPIX, PINS> {
                type Error = Error;
            ...
            impl<PINS> crate::hal::blocking::spi::transfer::Default<u8> for UncheckedSpi<$SPIX, PINS> {}

            impl<PINS> crate::hal::blocking::spi::write::Default<u8> for UncheckedSpi<$SPIX, PINS> {}

            impl<PINS> crate::hal::blocking::spi::transfer::Default<u16> for UncheckedSpi<$SPIX, PINS> {}

            impl<PINS> crate::hal::blocking::spi::write::Default<u16> for UncheckedSpi<$SPIX, PINS> {}
        )+
    }
}
